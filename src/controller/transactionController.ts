...
const createTransaction = async (
    ...
    try {
        ...
        for (let index = 0; index < transaction_detail.length; index++) {
            ...
            const medecineItem = findMedicine.find(item => item.id == medicine_id)
            /**push data to array */
            newDetail.push({
                transaction_id: newTransaction.id,
                medicine_id,
                qty,
                order_price: medecineItem?.price || 0
            })
        }
...
const readTransaction = async (req: Request, res: Response) => {
    try {
        ...
        /** get all transaction data and details on each transaction */
        let allTransaction = await prisma
            .transaction.findMany({
                include: { transaction_detail: { include: { medicine_detail: true } } },
        ...
        if (req.query.start_date && req.query.end_date) {
            allTransaction = allTransaction
                .filter(
                    trans => trans.order_date >= start_date
                    &&
        ...
        allTransaction = allTransaction
            .map(trans => {
                let total = trans.transaction_detail
                    .reduce((jumlah, detail) => jumlah + (detail.order_price * detail.qty),
                        0)
                return {
                    ...trans, total
                }
...
const deleteTransaction = async (req: Request, res: Response) => {
    try {
        ...
        /**hapus detail transaksi dulu,karena detail transaksi 
         * adalah tabel yang tergantung pada tabel transaksi
         */
        await prisma.transaction_detail
            .deleteMany({
